      ...
      <div className="min-h-screen bg-surface p-6">
        <div className="max-w-xl mx-auto space-y-6">
          <div className="flex items-center gap-3">
            <button onClick={() => { setView('main'); setSearchResults([]); setSearchQuery(''); }} className="text-slate-400 hover:text-white transition-colors text-sm font-semibold">← Back</button>
          ...
          <div className="bg-surface-card border border-surface-border rounded-2xl p-5 space-y-4">
            <p className="text-slate-400 text-sm">Search for a TheWileyfox user to add as your emergency contact. They will receive a request to accept.</p>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-surface border border-surface-border text-white text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:border-brand-500 placeholder:text-slate-500"
              />
              <button
                onClick={search}
      ...
      <div className="max-w-xl mx-auto space-y-6">
        ...
        <div className="bg-red-500/10 border border-red-500/30 rounded-3xl p-6 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Instantly alerts all accepted contacts with your GPS location.</p>
          </div>
          <textarea
            value={sosMessage}
            ...
            rows={2}
            maxLength={500}
            className="w-full bg-surface border border-surface-border text-white text-sm rounded-xl px-3 py-2.5 resize-none focus:outline-none focus:border-red-500/60 placeholder:text-slate-600"
          />
          <button
            onClick={triggerSos}
            disabled={sosLoading}
          ...
          {accepted.length === 0 && !loading && (
            <p className="text-slate-500 text-xs">Add accepted contacts to enable SOS</p>
          )}
        </div>
            ...
            {accepted.map((c) => (
              <div key={c.id} className="bg-surface-card border border-surface-border rounded-2xl p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-500 text-xs truncate">{c.contact?.email}</p>
                </div>
                <button onClick={() => removeContact(c.id)} className="text-slate-600 hover:text-red-400 transition-colors">
