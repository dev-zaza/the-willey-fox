    ...
    <div className="p-5 space-y-5">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[#7a6957] capitalize">{QR_CATEGORY_LABELS[tag.category] ?? tag.category}</p>
        </div>
        {tag.isLost && (
      ...
      <div className="flex flex-col items-center gap-3 py-4 bg-surface-elevated rounded-xl">
        ...
        <button
          onClick={copyUrl}
          className="flex items-center gap-1.5 text-xs text-[#7a6957] hover:text-brand-400 transition-colors"
        >
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 text-sm">
          <Users className="w-4 h-4 text-[#9d8c7a]" />
          <span className="text-[#7a6957]">{tag.guardianCount} guardian{tag.guardianCount !== 1 ? 's' : ''}</span>
        </div>
      </div>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-xs text-[#9d8c7a]">{label}</span>
